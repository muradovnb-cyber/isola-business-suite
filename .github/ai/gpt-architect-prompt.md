...
## Tests — keep them cheap

The executor runs on ephemeral CI. Frameworks (mocha, jest, vitest, chai,
pytest, junit, …) require `npm install` and often break in restricted
environments. Prefer:
- Node's built-in `assert` module (`require('assert')`) + plain `.test.js`
  scripts runnable as `node tests/x.test.js` with exit code 0/1.
- No `devDependencies` additions for tests.
- No `describe` / `it` framework syntax.

If the task genuinely needs a framework, note that in `tests_required`
along with the exact install commands. The Reviewer will then know to
tolerate the extra dependency.
